// Neutronics/heat conduction test case (hp-adaptive version).
//
// Authors: Damien Lebrun-Grandie (Texas A&M University, USA),
//          Milan Hanus (University of West Bohemia, Pilsen, Czech Rep.).
// PDEs:
//
// d/dt T = div(lambda(T) grad T) + kappa/(rho*c_p) * Sigma_f * phi + qT/(rho*c_p);  
// (lambda(T) = k(T) / (rho*c_p))
//
// d/dt phi = div(v * D * grad phi) + v * (nu * Sigma_f - Sigma_r(T)) * phi + v * qF
//
...
const double T_FINAL = 100*TIME_STEP;      // Time interval length (secs.).
...
int main(int argc, char* argv[])
{
  ...
  // Initialize solution and order views.
  ViewsAdapt results_views(450, 350);
  
  // Show initial solutions.
  cpu_time.tick();
  results_views.show_solutions(current_time, prev_time_solutions);
  results_views.show_exact(current_time, exact_solutions);
  ...
  do 
  {
    ...
    do 
    {
      ...
      // Time for which rk_time_step returned the new solution.
      double next_time = current_time + TIME_STEP;
      
      // Visualize intermediate solutions and mesh during adaptivity.  
      results_views.show_solutions(next_time, as, coarse_mesh_solutions);
      results_views.show_orders(next_time, as, spaces);
    ...
    results_views.show_solutions(current_time, coarse_mesh_solutions);
    results_views.show_orders(current_time, spaces);
