...
public:
                       ...
                       double* current_time_ptr, double temp_init, double t_final) : WeakForm(1)
  {
    ...
    add_matrix_form(new DefaultJacobianDiffusion(0, 0, HERMES_ANY, new HermesFunction(-lambda / (heatcap * rho))));

    // Jacobian surface part.
    add_matrix_form_surf(new DefaultMatrixFormSurf(0, 0, bdy_air, new HermesFunction(-alpha / (heatcap * rho))));

    // Residual - volumetric.
    add_vector_form(new DefaultResidualDiffusion(0, HERMES_ANY, new HermesFunction(-lambda / (heatcap * rho))));
