...
int main(int argc, char* argv[])
{
  ...
  Mesh mesh;
  H2DReader mloader;
  mloader.load("triangle.mesh", &mesh);
  ...
  CustomExactSolution sln_in(&mesh);

  OGProjection::project_global(&space, &sln_in, coeff_vec, matrix_solver, HERMES_L2_NORM);
  ...
  ScalarView c("Initial solution.");
  c.show(&sln);
  c.wait_for_close();
  */
  // Perform Newton's iteration.

  if (!hermes2d.solve_newton(coeff_vec, &dp, solver, matrix, rhs, true, 1e-04)) 
